...
import {
  ...
  Briefcase, Car, Users, Home, Scale,
  Loader2, FolderOpen, Brain,
} from 'lucide-react'
...
const COLOR_MAP = {
  blue:   { card: 'border-blue-500/40 bg-blue-500/8',   icon: 'bg-blue-500/15 text-blue-400',   ring: 'ring-blue-500/40'   },
  orange: { card: 'border-orange-500/40 bg-orange-500/8', icon: 'bg-orange-500/15 text-orange-400', ring: 'ring-orange-500/40' },
  green:  { card: 'border-green-500/40 bg-green-500/8', icon: 'bg-green-500/15 text-green-400',  ring: 'ring-green-500/40'  },
  purple: { card: 'border-purple-500/40 bg-purple-500/8',icon: 'bg-purple-500/15 text-purple-400',ring: 'ring-purple-500/40' },
  gold:   { card: 'border-gold-500/40 bg-gold-500/8',   icon: 'bg-gold-500/15 text-gold-400',    ring: 'ring-gold-500/40'   },
}
      ...
      >
        ...
        <div className="text-center">
          ...
          <p className="text-xs text-[var(--text-muted)] mt-1">
            or <span className="text-gold-400 underline underline-offset-2">browse from your device</span>
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 text-[10px] text-[var(--text-muted)] uppercase tracking-widest mt-1">
          {['PDF', 'JPG', 'PNG', 'DOCX'].map(ext => (
            <span key={ext} className="px-2 py-0.5 rounded bg-white/4 border border-white/6">{ext}</span>
          ))}
        </div>
      </div>
          ...
          <div className="h-1.5 rounded-full bg-white/8 overflow-hidden">
            <div
              className={clsx(
                ...
                pct === -1  ? 'bg-red-500' :
                pct === 100 ? 'bg-green-400' : 'bg-gold-500',
              )}
  ...
  return (
    <div className="flex flex-col items-center justify-center py-8 px-4 text-center animate-slide-up">
        ...
        {[
          ...
          <div key={i} className="flex items-center gap-3 text-sm">
            <div className={clsx(
              ...
              s.done   ? 'bg-green-500/15 border border-green-500/30 text-green-400' :
              s.active ? 'bg-gold-500/15  border border-gold-500/30  text-gold-400'  :
                         'bg-white/5      border border-[var(--border)] text-[var(--text-muted)]',
            )}>
              {s.done   ? <Check size={10} strokeWidth={3} /> :
               s.active ? <span className="w-1.5 h-1.5 rounded-full bg-current animate-pulse" /> :
                          <span className="w-1.5 h-1.5 rounded-full bg-current opacity-30" />}
            </div>
            <span className={s.done || s.active ? 'text-[var(--text-primary)]' : 'text-[var(--text-muted)]'}>
              {s.label}
            </span>
          </div>
        ))}
      ...
      <div className="flex flex-col sm:flex-row gap-2 w-full max-w-xs">
        ...
        <Button onClick={onClose} variant="secondary" size="md" icon={FolderOpen} className="flex-1">
          My Cases
        </Button>
      </div>
    </div>
      ...
      <div className={clsx(
          ...
          <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-[var(--border)] shrink-0">
            <div>
              ...
              <p className="text-xs text-[var(--text-muted)] font-medium">
                Step {step + 1} of 3 — <span className="text-[var(--text-secondary)]">{STEP_LABELS[step]}</span>
              </p>
            </div>
            <button
              type="button"
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5">
          {done ? (
            ...
            <>
              {step === 0 && (
                <StepType selected={caseType} onSelect={setCaseType} />
              )}
              {step === 1 && (
                <StepDescription
                  value={description}
              ...
              {step === 2 && (
                <StepDocuments files={files} onFilesChange={setFiles} />
              )}
