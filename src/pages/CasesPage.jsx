...
import {
  ...
  FileText, HelpCircle, X, ChevronDown, Brain
} from 'lucide-react'
...
function CaseRow({ caseItem, t }) {
  ...
  return (
    <div
      ...
      className="group grid grid-cols-[2.5rem_1fr_auto_auto] sm:grid-cols-[2.5rem_1fr_8rem_8rem_6rem_5rem] items-center gap-4 px-4 py-3.5 rounded-xl bg-[var(--bg-card)] border border-[var(--border)] hover:border-white/10 hover:bg-[var(--bg-elevated)] transition-all duration-200 cursor-pointer"
    >
      {/* Icon */}
        ...
        {caseItem.ai_status === 'done' && caseItem.ai_risk_level
          ...
          : caseItem.ai_status === 'processing'
            ...
            : <span className="text-[10px] text-[var(--text-muted)] flex items-center gap-1">
                <Brain size={10} className="opacity-40" /> Pending
              </span>
        }
      </div>
      ...
      {/* Arrow — desktop */}
      <button className="hidden sm:block text-xs text-[var(--text-muted)] hover:text-gold-400 transition-colors opacity-0 group-hover:opacity-100 text-end shrink-0">
        {t('case.viewDetails')} →
      </button>
    </div>
  )
...
function EmptyFiltered({ onClear }) {
  return (
    <div className="flex flex-col items-center py-16 text-center">
      ...
      <p className="text-sm font-medium text-[var(--text-primary)] mb-1">No cases match your filters</p>
      <p className="text-xs text-[var(--text-muted)] mb-4">Try adjusting the status or search term</p>
      <button onClick={onClear} className="text-xs text-gold-400 hover:text-gold-300 transition-colors flex items-center gap-1">
...
function Skeleton() {
  return (
    <div className="space-y-2">
      {[1,2,3,4,5].map(i => (
        <div key={i} className="grid grid-cols-[2.5rem_1fr_auto] items-center gap-4 px-4 py-3.5 rounded-xl bg-[var(--bg-card)] border border-[var(--border)] animate-pulse">
...
export function CasesPage() {
  ...
  const onCreated = () => { setShowModal(false); refetch() }
  ...
  return (
    <>
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 animate-slide-up">
          <div>
            ...
            <p className="text-sm text-[var(--text-secondary)] mt-1">
              {loading ? '—' : `${cases.length} total · ${stats.active} active · ${stats.pending} pending`}
            </p>
          </div>
      ...
      {showModal && (
        <CreateCaseModal onClose={() => setShowModal(false)} onCreated={onCreated} />
      )}
    </>
  )
}
