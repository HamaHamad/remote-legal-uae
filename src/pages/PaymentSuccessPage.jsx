...
export function PaymentSuccessPage() {
  ...
  const [stage,   setStage]   = useState(0)   // 0=verifying, 1=unlocking, 2=done
  const [failed,  setFailed]  = useState(false)

  useEffect(() => {
    if (!caseId) { setFailed(true); return }
    ...
    const run = async () => {
      ...
      if (unlocked) {
        ...
        // user can refresh the case page
        setStage(2)
      }
    }
      ...
      <div className="min-h-screen auth-bg flex items-center justify-center px-4">
        <div className="glass-panel gold-border rounded-2xl p-8 max-w-sm w-full text-center animate-slide-up">
          ...
          <p className="text-sm text-[var(--text-secondary)] mb-6">
            We couldn't identify your case. Your payment may still have been processed — please check your email or contact support.
          </p>
          <Link to="/dashboard">
            <Button icon={Home} fullWidth>Back to Dashboard</Button>
          </Link>
        </div>
      </div>
    )
