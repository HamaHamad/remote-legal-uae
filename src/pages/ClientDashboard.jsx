...
import {
  FolderOpen, Plus, FileText, Clock, CheckCircle,
  Activity, ArrowRight, Sparkles, Briefcase,
  Car, Users, Home, Scale, HelpCircle
} from 'lucide-react'
    ...
    <div className="flex flex-col items-center justify-center py-16 px-4 animate-fade-in">
      ...
      <div className="mt-10 grid grid-cols-1 sm:grid-cols-3 gap-3 w-full max-w-xl">
        {[
          ...
          <div key={item.label} className="flex flex-col items-center gap-2 p-4 rounded-xl bg-white/2 border border-white/5 text-center">
...
function CaseCard({ caseItem, t }) {
  ...
  return (
    <div
      ...
      className="group flex flex-col sm:flex-row sm:items-center gap-4 px-4 py-4 rounded-xl bg-[var(--bg-card)] border border-[var(--border)] hover:border-white/10 hover:bg-[var(--bg-elevated)] transition-all duration-200 cursor-pointer"
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {caseItem.description
          ? <p className="text-xs text-[var(--text-secondary)] mt-0.5 line-clamp-1">{caseItem.description}</p>
          : <p className="text-xs text-[var(--text-muted)] mt-0.5 italic">{t('case.noDescription')}</p>
        }
      </div>
      ...
      <ArrowRight size={14} className="hidden sm:block text-[var(--text-muted)] opacity-0 group-hover:opacity-60 transition-opacity shrink-0" />
    </div>
  )
}
...
function CaseSkeleton() {
  return (
    <div className="space-y-2">
      {[1,2,3].map(i => (
        <div key={i} className="flex items-center gap-4 px-4 py-4 rounded-xl bg-[var(--bg-card)] border border-[var(--border)] animate-pulse">
...
export function ClientDashboard() {
  ...
  const onCreated  = () => { closeModal(); refetch() }
    ...
    <>
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 animate-slide-up-delay-1">
          <StatCard label={t('dashboard.totalCases')}    value={loading ? '—' : stats.total}    icon={FolderOpen}  color="gold"   />
          <StatCard label={t('dashboard.activeCases')}   value={loading ? '—' : stats.active}   icon={Activity}    color="green"  />
          <StatCard label={t('dashboard.pendingCases')}  value={loading ? '—' : stats.pending}  icon={Clock}       color="blue"   />
          <StatCard label={t('dashboard.resolvedCases')} value={loading ? '—' : stats.resolved} icon={CheckCircle} color="purple" />
        </div>
        ...
        <div className="animate-slide-up-delay-2">
          <div className="flex items-center justify-between mb-4">
            ...
            {cases.length > 0 && (
              <Link to="/dashboard/cases" className="text-xs text-gold-400 hover:text-gold-300 transition-colors flex items-center gap-1">
...
function QuickAction({ icon: Icon, label, desc, gold, disabled, onClick }) {
  return (
    <div
      ...
      className={clsx(
        'flex items-center gap-4 p-4 rounded-xl border transition-all duration-200 h-full',
        disabled   ? 'opacity-40 cursor-not-allowed border-[var(--border)] bg-[var(--bg-card)]'
        : gold     ? 'cursor-pointer border-gold-500/30 bg-gold-500/5 hover:bg-gold-500/10 hover:border-gold-500/50 hover:shadow-gold-sm'
    ...
    >
      ...
      <div>
        <p className={clsx('text-sm font-semibold', gold ? 'text-gold-300' : 'text-[var(--text-primary)]')}>{label}</p>
