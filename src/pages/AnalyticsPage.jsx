  ...
  return (
    <div className="glass-panel rounded-xl p-5 flex items-start justify-between">
      <div>
        ...
        {trend !== undefined && (
          <p className={clsx('text-xs mt-1 flex items-center gap-1', trend >= 0 ? 'text-green-400' : 'text-red-400')}>
            {trend >= 0 ? <ArrowUp size={10} /> : <ArrowDown size={10} />}
            {Math.abs(trend)}% vs last month
          </p>
        )}
      </div>
      <div className={clsx('w-10 h-10 rounded-lg border flex items-center justify-center shrink-0', colorMap[color])}>
        <Icon size={18} />
      </div>
    </div>
  )
...
function BarChart({ data, label }) {
  ...
  return (
    <div>
      <p className="text-[10px] font-semibold text-[var(--text-muted)] uppercase tracking-widest mb-4">{label}</p>
...
function TypeBreakdown({ cases }) {
  ...
  const colors = [
    ...
    'bg-purple-500', 'bg-orange-500', 'bg-cyan-500',
  ]
      ...
      {sorted.map(([type, count], i) => (
        <div key={type}>
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-xs text-[var(--text-muted)]">{count} ({Math.round(count/total*100)}%)</span>
          </div>
          <div className="h-1.5 rounded-full bg-white/5 overflow-hidden">
            <div
              className={clsx('h-full rounded-full transition-all duration-700', colors[i % colors.length])}
...
function RiskPie({ cases }) {
  ...
  aiDone.forEach(c => { if (c.ai_risk_level) counts[c.ai_risk_level]++ })
  ...
  return (
    <div className="space-y-3">
      ...
      {aiDone.length === 0 && (
        <p className="text-xs text-[var(--text-muted)] text-center py-4">No AI analyses completed yet</p>
      )}
    </div>
  )
}
...
export function AnalyticsPage() {
  ...
  const aiReadyPct = s.total_cases > 0
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="flex items-end justify-between animate-slide-up">
        <div>
          <h1 className="font-display text-3xl font-semibold text-[var(--text-primary)]">Analytics</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 animate-slide-up-delay-1">
        <MetricCard label="Total Cases"    value={loading ? '—' : (s.total_cases    || 0)} icon={FolderOpen} color="gold"   />
        <MetricCard label="Total Users"    value={loading ? '—' : (s.total_users    || 0)} icon={Users}      color="blue"   />
        <MetricCard label="Paid Unlocks"   value={loading ? '—' : (s.paid_unlocks   || 0)} icon={DollarSign} color="green"  />
        <MetricCard label="Total Revenue"  value={loading ? '—' : revenueAED}              icon={TrendingUp} color="purple" />
      </div>

      {/* Secondary metrics */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 animate-slide-up-delay-1">
        <MetricCard label="Pending Cases"  value={loading ? '—' : (s.pending_cases  || 0)} icon={Clock}         color="gold"   />
        <MetricCard label="Active Cases"   value={loading ? '—' : (s.active_cases   || 0)} icon={TrendingUp}    color="blue"   />
        <MetricCard label="Resolved"       value={loading ? '—' : (s.resolved_cases || 0)} icon={CheckCircle}   color="green"  />
        <MetricCard label="AI Analyses"    value={loading ? '—' : `${aiReadyPct}%`}         icon={Brain}         color="purple" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-slide-up-delay-2">
        ...
        <div className="space-y-5">
          ...
          <div className="glass-panel rounded-xl p-4">
            <h3 className="text-[10px] font-semibold text-[var(--text-muted)] uppercase tracking-widest mb-3">Team</h3>
            <div className="space-y-3">
              {[
          ...
          {/* Task breakdown */}
          <div className="glass-panel rounded-xl p-4">
            <h3 className="text-[10px] font-semibold text-[var(--text-muted)] uppercase tracking-widest mb-3">Tasks</h3>
            <div className="space-y-3">
              {[
                { label: 'Total Tasks',  value: s.total_tasks     || 0, color: 'text-[var(--text-primary)]' },
                { label: 'Completed',    value: s.completed_tasks || 0, color: 'text-green-400' },
                { label: 'Completion %', value: s.total_tasks > 0 ? `${Math.round((s.completed_tasks||0)/s.total_tasks*100)}%` : '—', color: 'text-gold-400' },
              ].map(item => (
                <div key={item.label} className="flex items-center justify-between">
                  <span className="text-sm text-[var(--text-secondary)]">{item.label}</span>
