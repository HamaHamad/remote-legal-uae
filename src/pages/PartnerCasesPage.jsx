...
import {
  ...
  Search, X, RefreshCw, AlertCircle, Clock,
  ChevronRight, FileText, Brain, CheckCircle
} from 'lucide-react'
  ...
  return (
    <div className="glass-panel rounded-2xl p-5 hover:border-white/10 hover:bg-[var(--bg-elevated)] transition-all group">
      ...
      <div className="flex items-start gap-4 mb-4">
        <div className={clsx('w-11 h-11 rounded-xl flex items-center justify-center shrink-0', meta.bg)}>
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-end justify-between animate-slide-up">
        <div>
          <h1 className="font-display text-3xl font-semibold text-[var(--text-primary)]">Assigned Cases</h1>
      ...
      <div className="flex flex-wrap gap-3 animate-slide-up-delay-1">
        <div className="relative flex-1 min-w-[220px]">
          ...
          <input
            type="text"
            placeholder="Search by type, client, or case ID…"
            value={search}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 animate-fade-in">
          {filtered.map(c => (
            <CaseCard
              key={c.id}
              caseItem={c}
              taskCount={taskCountByCaseId[c.id]}
            />
          ))}
        </div>
      )}
